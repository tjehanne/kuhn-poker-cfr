...
class CRNN(nn.Module):
    def __init__(self, num_chars, hidden_size=256):
        ...
        # Input: 1 x 64 x 200 (GrayScale)
        # CNN pour extraire les features visuelles
        self.cnn = nn.Sequential(
            # Layer 1
            nn.Conv2d(1, 64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2, 2), # -> 64 x 32 x 100
            
            # Layer 2
            nn.Conv2d(64, 128, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2, 2), # -> 128 x 16 x 50
            
            # Layer 3
            nn.Conv2d(128, 256, kernel_size=3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.MaxPool2d((2, 1)), # -> 256 x 8 x 50 (On garde la largeur pour la séquence temporelle)
            
            # Layer 4
            nn.Conv2d(256, 512, kernel_size=3, padding=1),
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d((2, 1)), # -> 512 x 4 x 50
        )
        
        # RNN pour la séquence
        # Input features calculation: 512 channels * 4 height = 2048
        self.rnn = nn.LSTM(input_size=512 * 4, 
    ...
    def forward(self, x):
        ...
        # Ici TimeSteps = w (50), Features = c * h (512 * 4 = 2048)
